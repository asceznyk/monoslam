...
from utils import FundamentalMatrixTransform
...
ftf = FundamentalMatrixTransform()
etf = EssentialMatrixTransform()
src = np.random.rand(8,2)
dst = np.random.rand(8,2)
ftf.estimate(src, dst)
etf.estimate(src, dst)

fres = ftf.residuals(src, dst)
eres = etf.residuals(src, dst)
